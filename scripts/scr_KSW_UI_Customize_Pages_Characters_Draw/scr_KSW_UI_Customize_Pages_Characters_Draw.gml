///@description KSW - UI - Customize - Pages - Characters - Draw

function scr_KSW_UI_Customize_Pages_Characters_Draw()
{
	#region Variables
	...
	for (var i = startIndex; i < endIndex; i++)
	{
		...
		#region Sprite
		if (global.KSW_CharacterList[ds_list_find_value(selectionList,i)].icon != -1)
		{
			if ((ds_list_find_value(selectionList,i) != -1) and (!global.KSW_CharacterList[ds_list_find_value(selectionList,i)].isUnlocked)) gpu_set_fog(true,c_black,0,0);
		...
		if (!global.KSW_CharacterList[ds_list_find_value(selectionList,i)].isUnlocked)
		{
			if (global.KSW_CharacterList[ds_list_find_value(selectionList,i)].price == 0)
			{
			...
			else
			{
				...
				scribble(string(global.KSW_CharacterList[ds_list_find_value(selectionList,i)].price)).align(fa_center).draw(boxX +  34,boxY + 40);
			}
		}
		#endregion
	...
	#region Page Title
	scribble("CharacterS").align(fa_center).draw(xx + (global.gameWidth / 2),yy + 6 - hintOffset);
	#endregion
	...
	var targetName = "???";
	if (global.KSW_CharacterList[ds_list_find_value(selectionList,selection)].isUnlocked) targetName = global.KSW_CharacterList[ds_list_find_value(selectionList,selection)].name;
