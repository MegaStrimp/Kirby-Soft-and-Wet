...
function scr_KSW_UI_Customize_Pages_Baits_Draw()
{
	...
	#region Page Title
	scribble("BAITS " + string(page + 1)).align(fa_center).draw(xx + (global.gameWidth / 2),yy + 6 - hintOffset);
	#endregion
	...
	scribble(color + string(global.KSW_UnlockedBaitCount) + "/" + string(global.KSW_BaitCount) + "[/color]").align(fa_right).draw(global.gameWidth - 4,6 - hintOffset);
	#endregion
	...
	for (var i = (page * pageSelectionCount); i < min((page + 1) * pageSelectionCount,ds_list_size(selectionList)); i++)
	{
		...
		if (ds_list_find_value(selectionList,i) != -1)
		{
			...
			var spriteXOffset = global.KSW_BaitList[ds_list_find_value(selectionList,i)].xOffset;
			var spriteYOffset = global.KSW_BaitList[ds_list_find_value(selectionList,i)].yOffset;
			if (global.KSW_BaitList[ds_list_find_value(selectionList,i)].isUnlocked) backgroundPalette = global.KSW_BaitList[ds_list_find_value(selectionList,i)].boxPalette;
		}
		else
		{
		...
		#region Box
		var isSelected = (ds_list_find_value(selectionList,i) == global.KSW_EquippedBaitID[playerNum]);
		...
		if (spriteIndex != -1)
		{
			if ((ds_list_find_value(selectionList,i) != -1) and (!global.KSW_BaitList[ds_list_find_value(selectionList,i)].isUnlocked)) gpu_set_fog(true,c_black,0,0);
			draw_sprite(spriteIndex,baitImageIndex[i],boxX + 14 + spriteXOffset + calibX,boxY + 14 + spriteYOffset + calibY);
			if ((ds_list_find_value(selectionList,i) != -1) and (!global.KSW_BaitList[ds_list_find_value(selectionList,i)].isUnlocked)) gpu_set_fog(false,c_black,0,0);
		}
		#endregion
		...
		if (!global.KSW_BaitList[ds_list_find_value(selectionList,i)].isUnlocked)
		{
			if (global.KSW_BaitList[ds_list_find_value(selectionList,i)].price == 0)
			{
			...
			else
			{
				...
				scribble(string(global.KSW_BaitList[ds_list_find_value(selectionList,i)].price)).align(fa_center).draw(boxX + 14,boxY + 17);
			}
		}
		#endregion
	...
	#region Bait Name
	var targetName = "???";
	if (global.KSW_BaitList[ds_list_find_value(selectionList,selection)].isUnlocked) targetName = global.KSW_BaitList[ds_list_find_value(selectionList,selection)].name;
	...
	if ((!global.KSW_BaitList[ds_list_find_value(selectionList,selection)].isUnlocked) and (global.KSW_BaitList[ds_list_find_value(selectionList,selection)].price != 0)) text = scribble(selectIcon + "BUY");
