///@description KSW - UI - Customize - Pages - Hats - Draw

function scr_KSW_UI_Customize_Pages_Hats_Draw()
{
	#region Variables
	...
	#region Page Title
	scribble("HATS " + string(page + 1)).align(fa_center).draw(xx + (global.gameWidth / 2),yy + 6 - hintOffset);
	#endregion
	...
	for (var i = (page * pageSelectionCount); i < min((page + 1) * pageSelectionCount,ds_list_size(selectionList)); i++)
	{
		...
		if (global.KSW_PlayerEquippedHatShuffle[playerNum])
		{
			isSelected = (ds_list_find_value(selectionList,i) == -1);
		}
		else
		{
			isSelected = (ds_list_find_value(selectionList,i) == global.KSW_PlayerEquippedHatID[playerNum][global.playerCharacter[playerNum]]);
		}
		...
		if ((ds_list_find_value(selectionList,i) != -1) and (!global.KSW_CharacterList[global.playerCharacter[playerNum]].hats[ds_list_find_value(selectionList,i)].isUnlocked))
		{
			if (global.KSW_CharacterList[global.playerCharacter[playerNum]].hats[ds_list_find_value(selectionList,i)].price == 0)
			{
			...
			else
			{
				...
				scribble(string(global.KSW_CharacterList[global.playerCharacter[playerNum]].hats[ds_list_find_value(selectionList,i)].price)).align(fa_center).draw(boxX + 14,boxY + 17);
			}
		}
		#endregion
	...
	{
		var targetName = "???";
		if (global.KSW_CharacterList[global.playerCharacter[playerNum]].hats[ds_list_find_value(selectionList,selection)].isUnlocked) targetName = global.KSW_CharacterList[global.playerCharacter[playerNum]].hats[ds_list_find_value(selectionList,selection)].name;
	}
